...
const countdown3 = ({props}) => {
  ...
  const count = useCssHandles(CSS_HANDLES)
   var id3 = new Number(props.idColect3);
   var id3S = String(id3);
   console.log("ID colect 3", id3);
  ...
  var fechaFin = new Date(props.enddate3).getTime();
  var fechaIni = new Date(props.startdate3).getTime();
  console.log("fechaFin 3:", fechaFin);
  console.log("fecha Ini 3", fechaIni);
  var startDate = moment(fechaIni);
  var endDateF = moment(fechaFin).format('MMMM DD YYYY, h:mm:ss')
  console.log("fecha FIn formato 3", endDateF);
  ...
  function mostrarOferta() {
    if (hide3 === "none" && terminado === false && horaF > horaA) {
      contador();
    }
  ...
  return (
    <div className={`${count.ofertaContainer}`} style={{ display: hide3 }}>
      <Helmet>
        ...
        <div id="countdown-body" className={`${count.countdownbody}`}>
          ...
          <div className={`${count.rowRight}`} id="outline-countdown">
            <div className={`${count.headerRowRight}`}>
              <div id="hours3" className={`${count.hours}`}></div>
              <p className={`${count.separador}`}>:</p>
              <div id="minutes3" className={`${count.minutes}`}></div>
              <p className={`${count.separador}`}>:</p>
              <div id="seconds3" className={`${count.seconds}`}></div>
            </div>
          </div>
